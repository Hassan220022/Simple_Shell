...
/**
 * _erra_toi - converts a string to an integer
 * @s: the string to be converted
 * Return: 0 if no numbers in string, converted number otherwise
 *       -1 on error
 */
int _erra_toi(char *s)
{
	int i = 0;
 ...
 *        -1 on error
 */
void _print_error(command_info *info, char *estr)
{
	error_puts(info->file_name);
	error_puts(": ");
	print_decimal(info->line_counter, STDERR_FILENO);
	error_puts(": ");
	error_puts(info->input_arr[0]);
	error_puts(": ");
	error_puts(estr);
}
...
/**
 * convert_number_itoa - converter function, a clone of itoa
 * @num: number
 * @base: base
 ...
 * Return: string
 */
char *convert_number_itoa(long int num, int base, int flags)
{
	static char *array;
